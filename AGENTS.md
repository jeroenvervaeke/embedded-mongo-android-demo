...
- **The two map buttons are icons, bottom right.** A crosshair frames the radius, an Ireland
  outline frames the island — both drawn in code, the island from a coarse ring of real
...
explorer and its `EXPLAIN`, and the about screen. Tested on the JVM as well — `:data:test` and
